...
func TestMigrations_IdentityAndHierarchyPreservesIDsAndBackfillsDefaults(t *testing.T) {
	ctx := context.Background()
	fixture := setupMigrationFixture(t)
	...
	_, err := pool.Exec(ctx,
		`INSERT INTO tenants (id, name) VALUES ($1, 'migration-account')`, tenantID)
	require.NoError(t, err)
	_, err = pool.Exec(ctx,
		`INSERT INTO users (id, email, name) VALUES ($1, 'legacy@example.test', 'Legacy User')`, userID)
	require.NoError(t, err)

	tenantIDsBefore := snapshotIDs(t, ctx, pool, `SELECT id FROM tenants ORDER BY id`)
	userIDsBefore := snapshotIDs(t, ctx, pool, `SELECT id FROM users ORDER BY id`)

	fixture.migrateTo(t, 20)

	requireIDsUnchanged(t, tenantIDsBefore, snapshotIDs(t, ctx, pool, `SELECT id FROM tenants ORDER BY id`))
	requireIDsUnchanged(t, userIDsBefore, snapshotIDs(t, ctx, pool, `SELECT id FROM users ORDER BY id`))

	var organizationCount int
	err = pool.QueryRow(ctx,
		`SELECT count(*) FROM organizations WHERE tenant_id = $1 AND is_default`, tenantID,
	).Scan(&organizationCount)
	require.NoError(t, err)
	assert.Equal(t, 1, organizationCount)

	var principal domain.Principal
	err = pool.QueryRow(ctx,
		`SELECT id, display_name, email, status FROM principals WHERE id = $1`, userID,
	).Scan(&principal.ID, &principal.DisplayName, &principal.Email, &principal.Status)
	require.NoError(t, err)
	assert.Equal(t, userID, principal.ID)
	assert.Equal(t, "Legacy User", principal.DisplayName)
	assert.Equal(t, "legacy@example.test", principal.Email)
	assert.Equal(t, domain.PrincipalStatusActive, principal.Status)

	var identityCount int
	err = pool.QueryRow(ctx, `SELECT count(*) FROM principal_identities`).Scan(&identityCount)
	require.NoError(t, err)
...
// ---------------------------------------------------------------------------
// Identity and hierarchy repositories
// ---------------------------------------------------------------------------

func TestIdentityRepositories_CreateLinkAndResolve(t *testing.T) {
	pool := setupTestDB(t)
	ctx := context.Background()
	tenant := createTestTenant(t, ctx, pool, "identity-account")
	...
	require.NoError(t, principalRepo.Create(ctx, principal))
	require.NotEmpty(t, principal.ID)

	identity := &domain.PrincipalIdentity{
		PrincipalID:     principal.ID,
		Provider:        "clerk",
		ExternalSubject: "user_external",
	}
	require.NoError(t, principalRepo.LinkIdentity(ctx, identity))

	resolvedPrincipal, err := principalRepo.GetByIdentity(ctx, "clerk", "user_external")
	require.NoError(t, err)
	...
	link := &domain.TenantIdentityLink{
		TenantID:   tenant.ID,
		Provider:   "clerk",
		ExternalID: "org_external",
	}
	require.NoError(t, linkRepo.Create(ctx, link))

	resolvedLink, err := linkRepo.GetByExternalID(ctx, "clerk", "org_external")
	require.NoError(t, err)
	assert.Equal(t, tenant.ID, resolvedLink.TenantID)

	duplicate := &domain.TenantIdentityLink{
		TenantID:   tenant.ID,
		Provider:   "clerk",
...
func TestHierarchyRepositories_EnforceTenantAndOrganizationScope(t *testing.T) {
	pool := setupTestDB(t)
	ctx := context.Background()
	tenantA := createTestTenant(t, ctx, pool, "hierarchy-a")
	tenantB := createTestTenant(t, ctx, pool, "hierarchy-b")
	...
	_, err := organizationRepo.GetByID(ctx, tenantB.ID, organization.ID)
	...
	_, err = teamRepo.GetByID(ctx, tenantB.ID, organization.ID, team.ID)
	require.ErrorIs(t, err, domain.ErrTeamNotFound)

	forgedTeam := &domain.Team{
		TenantID:       tenantB.ID,
		OrganizationID: organization.ID,
		Name:           "Forged",
	}
	require.Error(t, teamRepo.Create(ctx, forgedTeam), "composite foreign key must reject cross-Tenant ancestry")
}

func TestMembershipRepositories_RoundTrip(t *testing.T) {
	pool := setupTestDB(t)
	ctx := context.Background()
	tenant := createTestTenant(t, ctx, pool, "membership-account")

	principalRepo := postgres.NewPrincipalRepository(pool)
	principal := &domain.Principal{
		DisplayName: "Grace Viewer",
	...
	organizationMembers := postgres.NewOrganizationMembershipRepository(pool)
	membership := &domain.OrganizationMembership{
		TenantID:       tenant.ID,
		OrganizationID: organization.ID,
		PrincipalID:    principal.ID,
		Role:           domain.OrganizationRoleViewer,
	}
	require.NoError(t, organizationMembers.Upsert(ctx, membership))
	membership.Role = domain.OrganizationRoleOperator
	require.NoError(t, organizationMembers.Upsert(ctx, membership))

	gotMembership, err := organizationMembers.Get(ctx, tenant.ID, organization.ID, principal.ID)
	require.NoError(t, err)
	assert.Equal(t, domain.OrganizationRoleOperator, gotMembership.Role)
	...
	teamMembers := postgres.NewTeamMembershipRepository(pool)
	teamMembership := &domain.TeamMembership{
		TenantID:       tenant.ID,
		OrganizationID: organization.ID,
		TeamID:         team.ID,
		PrincipalID:    principal.ID,
	}
	require.NoError(t, teamMembers.Upsert(ctx, teamMembership))
	require.NoError(t, teamMembers.Upsert(ctx, teamMembership), "membership upsert must be idempotent")

	isMember, err := teamMembers.IsMember(ctx, tenant.ID, organization.ID, team.ID, principal.ID)
	require.NoError(t, err)
	assert.True(t, isMember)

	require.NoError(t, teamMembers.Delete(ctx, tenant.ID, organization.ID, team.ID, principal.ID))
	isMember, err = teamMembers.IsMember(ctx, tenant.ID, organization.ID, team.ID, principal.ID)
	require.NoError(t, err)
