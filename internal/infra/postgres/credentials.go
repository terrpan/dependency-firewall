...
// DataPlaneCredentialRepository models a data plane credential repository.
type DataPlaneCredentialRepository struct{ pool *pgxpool.Pool }

// NewDataPlaneCredentialRepository constructs a new DataPlaneCredentialRepository.
...
// GetByID performs the get by id operation.
func (r *DataPlaneCredentialRepository) GetByID(
	ctx context.Context,
	tenantID, id string,
) (*domain.DataPlaneCredential, error) {
	return scanCredential(
		r.pool.QueryRow(
			ctx,
			`SELECT id,tenant_id,organization_id,COALESCE(team_id::text,''),name,secret_digest,expires_at,revoked_at,created_by,last_used_at,created_at,updated_at FROM data_plane_credentials WHERE tenant_id=$1 AND id=$2`,
			tenantID,
			id,
		),
	)
...
// ListByScope performs the list by scope operation.
func (r *DataPlaneCredentialRepository) ListByScope(
	ctx context.Context,
	tenantID, organizationID, teamID string,
) ([]domain.DataPlaneCredential, error) {
	rows, err := r.pool.Query(
		ctx,
		`SELECT id,tenant_id,organization_id,COALESCE(team_id::text,''),name,secret_digest,expires_at,revoked_at,created_by,last_used_at,created_at,updated_at FROM data_plane_credentials WHERE tenant_id=$1 AND organization_id=$2 AND team_id IS NOT DISTINCT FROM NULLIF($3,'')::uuid ORDER BY name`,
		tenantID,
		organizationID,
		teamID,
	)
...
// ListVerifiersByTenant performs the list verifiers by tenant operation.
func (r *DataPlaneCredentialRepository) ListVerifiersByTenant(
	ctx context.Context,
	tenantID string,
) ([]domain.DataPlaneCredentialVerifier, error) {
	rows, err := r.pool.Query(
		ctx,
		`SELECT id,tenant_id,organization_id,COALESCE(team_id::text,''),secret_digest,expires_at,revoked_at FROM data_plane_credentials WHERE tenant_id=$1`,
		tenantID,
	)
	if err != nil {
...
// Revoke performs the revoke operation.
func (r *DataPlaneCredentialRepository) Revoke(ctx context.Context, tenantID, id string, at time.Time) error {
	ct, err := r.pool.Exec(
		ctx,
		`UPDATE data_plane_credentials SET revoked_at=$1, updated_at=now() WHERE tenant_id=$2 AND id=$3`,
		at,
		tenantID,
		id,
	)
