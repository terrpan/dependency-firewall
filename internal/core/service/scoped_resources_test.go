...
func (a *recordingResourceAuthorizer) Authorize(_ context.Context, _ domain.AuthenticatedPrincipal, permission domain.Permission, scope domain.AuthorizationScope) error {
...
func (r *memoryScopedPolicyRepository) ListByOrganization(_ context.Context, tenantID, organizationID string) ([]domain.Policy, error) {
	result := []domain.Policy{}
	for _, policy := range r.listPolicies {
		if policy.TenantID == tenantID && policyOwnedByScope(policy, domain.AuthorizationScope{TenantID: tenantID, OrganizationID: organizationID}) {
			result = append(result, policy)
		}
	}
	return result, nil
...
func (r *memoryScopedPolicyRepository) ListEffective(ctx context.Context, scope domain.AuthorizationScope) ([]domain.Policy, error) {
...
func (r *memoryScopedPolicyRepository) GetEffectiveByID(ctx context.Context, scope domain.AuthorizationScope, id string) (*domain.Policy, error) {
...
func TestScopedPolicyService_SetsOwnershipAndRejectsInheritedMutation(t *testing.T) {
	...
	principal := domain.AuthenticatedPrincipal{Principal: domain.Principal{ID: "principal-1"}, TenantID: "tenant-1", TenantRole: domain.TenantRoleAdmin}
	...
	require.NoError(t, service.CreateOrganization(context.Background(), principal, "organization-1", organizationPolicy))
...
func TestScopedPolicyService_DenialPreventsMutation(t *testing.T) {
	repository := &memoryScopedPolicyRepository{spyPolicyServiceRepo: &spyPolicyServiceRepo{}}
	authorizer := &recordingResourceAuthorizer{err: &domain.AuthorizationError{Reason: domain.AuthorizationDenialInsufficientPermission}}
	base := NewPolicyService(repository, &spyPolicyRevisionRepository{}, &spyPolicyDecisionCache{}, nil)
	service := NewScopedPolicyService(base, repository, authorizer)
	principal := domain.AuthenticatedPrincipal{Principal: domain.Principal{ID: "principal-1"}, TenantID: "tenant-1", TenantRole: domain.TenantRoleMember}
	policy := &domain.Policy{Name: "deny", Type: domain.PolicyTypeCVSSThreshold, Action: domain.PolicyActionDeny, SchemaVersion: 1, Config: &domain.CVSSThresholdPolicyConfig{MaxCVSS: ptrFloat64(7)}, Enabled: true}
...
func (r *memoryScopedUpstreamRepository) GetByEcosystem(context.Context, string, domain.EcosystemType) (*domain.Upstream, error) {
	return nil, domain.ErrUpstreamNotFound
}
...
func (r *memoryScopedUpstreamRepository) Update(_ context.Context, upstream *domain.Upstream) error {
	for i := range r.items {
		if r.items[i].TenantID == upstream.TenantID && r.items[i].ID == upstream.ID && upstreamOwnedByScope(r.items[i], domain.AuthorizationScope{TenantID: upstream.TenantID, OrganizationID: upstream.OrganizationID, TeamID: upstream.TeamID}) {
			r.items[i] = *upstream
			return nil
		}
	}
...
func (r *memoryScopedUpstreamRepository) GetVisibleByID(ctx context.Context, scope domain.AuthorizationScope, id string) (*domain.Upstream, error) {
...
func (r *memoryScopedUpstreamRepository) ListVisible(_ context.Context, scope domain.AuthorizationScope) ([]domain.Upstream, error) {
	result := []domain.Upstream{}
	for _, upstream := range r.items {
...
func (r *memoryScopedUpstreamRepository) ResolveVisibleByEcosystem(context.Context, domain.AuthorizationScope, domain.EcosystemType) (*domain.Upstream, error) {
	return nil, domain.ErrUpstreamNotFound
}
...
func TestScopedUpstreamService_ListsOnlyOwnedScope(t *testing.T) {
	repository := &memoryScopedUpstreamRepository{items: []domain.Upstream{
		...
		{ID: "organization", TenantID: "tenant-1", OrganizationID: "organization-1", ScopeKind: domain.UpstreamScopeOrganizationShared},
		{ID: "team", TenantID: "tenant-1", OrganizationID: "organization-1", TeamID: "team-1", ScopeKind: domain.UpstreamScopeTeamLocal},
	}}
	authorizer := &recordingResourceAuthorizer{}
	service := NewScopedUpstreamService(NewUpstreamService(repository, nil), repository, authorizer)
	principal := domain.AuthenticatedPrincipal{Principal: domain.Principal{ID: "principal-1"}, TenantID: "tenant-1", TenantRole: domain.TenantRoleAdmin}

	organization, err := service.List(context.Background(), principal, domain.AuthorizationScope{TenantID: "tenant-1", OrganizationID: "organization-1"})
	require.NoError(t, err)
	require.Len(t, organization, 1)
	assert.Equal(t, "organization", organization[0].ID)

	team, err := service.List(context.Background(), principal, domain.AuthorizationScope{TenantID: "tenant-1", OrganizationID: "organization-1", TeamID: "team-1"})
	require.NoError(t, err)
