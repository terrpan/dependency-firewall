...
import {
  ...
  normalizeGraphRoots,
  useDependencyGraph,
  useDependencyGraphRoots,
} from '../features/dependency-graphs/api.ts'
...
function GraphMap({
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force(
        'collision',
        d3.forceCollide<D3GraphNode>().radius((node) => nodeRadius(node) + 21),
      )
    ...
    const node = nodeGroup
      ...
      .attr(
        'aria-label',
        (item) =>
          `${packageName(item)} version ${item.artifact.version}, ${nodeKind(item)} dependency at depth ${item.min_depth}`,
      )
      .attr('tabindex', 0)
      .on('click', (event, item) => {
    ...
    node
      ...
      .text(
        (item) =>
          `${packageName(item)}@${item.artifact.version} | ${nodeKind(item)} dependency | depth ${item.min_depth}`,
      )
    node.append('circle').attr('class', graphClass('graph-node-circle')).attr('r', nodeRadius)
    node
      .append('text')
  ...
  useEffect(() => {
    ...
    d3.select(svgRef.current)
      .selectAll<SVGGElement, D3GraphNode>(`.${graphClass('graph-node')}`)
    ...
    d3.select(svgRef.current)
      .selectAll<SVGGElement, D3GraphLink>(`.${graphClass('graph-edge-group')}`)
  ...
  return (
    <div className={graphClass('graph-map-stage')}>
      <div className={graphClass('graph-zoom-controls')} role="group" aria-label="Graph zoom controls">
        <button
          aria-label="Zoom out"
        ...
        <button
          ...
          className={graphClass('graph-zoom-level')}
          onClick={resetZoom}
          title="Reset zoom"
          type="button"
        >
      ...
      <div className={graphClass('graph-map-scroll')}>
        <svg
          ref={svgRef}
          className={graphClass('graph-map')}
          role="group"
...
function GraphWorkspace({ graph }: { graph: DependencyGraph }) {
  ...
  const selectedNode =
    selection?.kind === 'node' ? (visibleNodes.find((node) => node.id === selection.id) ?? null) : null
  const selectedEdge =
    selection?.kind === 'edge' ? (visibleEdges.find((edge) => edge.id === selection.id) ?? null) : null
  ...
  return (
    <>
      <div className={graphClass('graph-toolbar')}>
        <label className={graphClass('graph-search')}>
        ...
        <label className={graphClass('graph-depth-filter')}>
          <span>Show through depth</span>
          <select
            value={maxDepth ?? 'all'}
            onChange={(event) => {
              setMaxDepth(event.target.value === 'all' ? null : Number(event.target.value))
              setSelection(null)
            }}
          >
            <option value="all">All depths</option>
            <option value="1">Root + direct</option>
            <option value="2">Through depth 2</option>
            <option value="3">Through depth 3</option>
            <option value="4">Through depth 4</option>
          </select>
        </label>
        <div className={graphClass('graph-type-filters')} role="group" aria-label="Dependency types">
          {dependencyTypes.map((type) => (
            <button
              aria-pressed={activeTypes.includes(type)}
              className={graphClass(activeTypes.includes(type) && 'graph-type-active')}
              key={type}
              onClick={() => {
        ...
        {hasFilters ? (
          <button
            className={graphClass('secondary-button graph-clear-filters')}
            onClick={() => {
              setSearch('')
              setActiveTypes([])
              setMaxDepth(null)
      ...
      <div className={graphClass('graph-workspace-meta')}>
        <div className={graphClass('graph-legend')}>
          <span>
            <i className={graphClass('graph-legend-dot graph-legend-root')} />
            Root package
          </span>
          <span>
            <i className={graphClass('graph-legend-dot graph-legend-direct')} />
            Direct dependency
          </span>
          <span>
            <i className={graphClass('graph-legend-dot graph-legend-transitive')} />
            Transitive dependency
          </span>
          <span>
            <i className={graphClass('graph-legend-line')} />
            Relationship
          </span>
        </div>
        <p className={graphClass('graph-result-count')} aria-live="polite">
          Showing {visibleNodes.length} of {nodes.length} packages
        </p>
      </div>
      <p className={graphClass('graph-workspace-hint')}>
        Select a package or relationship for details. Drag packages to rearrange the map; use the controls or trackpad
        to zoom.
      </p>
      <div className={graphClass('graph-workspace-grid')}>
        <section className={graphClass('graph-map-card')} aria-label="Dependency relationship map">
          {visibleNodes.length ? (
            <GraphMap nodes={visibleNodes} edges={visibleEdges} selection={visibleSelection} onSelect={setSelection} />
          ) : (
            <QueryStateNotice
              title="No matching packages"
              message="Adjust the search, depth, or dependency type filters."
        ...
        {(selectedNode || selectedEdge) && (
          <aside className={graphClass('card graph-detail-card')}>
            <div className={graphClass('graph-detail-header')}>
              <p className={graphClass('eyebrow')}>{selectedEdge ? 'Selected relationship' : 'Selected package'}</p>
              <button
                aria-label="Close selection details"
                className={graphClass('secondary-button graph-detail-close')}
            ...
            {selectedNode ? (
              <>
                <h3>{packageName(selectedNode)}</h3>
                <p className={graphClass('graph-detail-version')}>{selectedNode.artifact.version}</p>
                <dl className={graphClass('graph-detail-list')}>
                  <div>
                    <dt>Role in this install</dt>
                    <dd>{nodeKind(selectedNode)}</dd>
                  </div>
                  <div>
                    <dt>Distance from root</dt>
                    <dd>
                      {selectedNode.min_depth === 0
                        ? 'Root package'
                        : `${selectedNode.min_depth} ${selectedNode.min_depth === 1 ? 'step' : 'steps'}`}
                    </dd>
                  </div>
                  <div>
                    <dt>Dependency type</dt>
                    <dd>{dependencyTypeList(selectedNode.dependency_types)}</dd>
                  </div>
                  <div>
                    <dt>Required by</dt>
                    <dd>{edges.filter((edge) => edge.child_node_id === selectedNode.id).length}</dd>
                  </div>
                  <div>
                    <dt>Dependencies</dt>
                    <dd>{edges.filter((edge) => edge.parent_node_id === selectedNode.id).length}</dd>
                  </div>
                </dl>
              ...
              <>
                <h3>
                  {packageName(nodesById.get(selectedEdge.parent_node_id) ?? nodes[0])}@
                  {nodesById.get(selectedEdge.parent_node_id)?.artifact.version}
                </h3>
                <p className={graphClass('muted graph-detail-arrow')}>depends on</p>
                <h3>
                  {packageName(nodesById.get(selectedEdge.child_node_id) ?? nodes[0])}@
                  {nodesById.get(selectedEdge.child_node_id)?.artifact.version}
                </h3>
                <dl className={graphClass('graph-detail-list')}>
                  <div>
                    <dt>Dependency type</dt>
                    <dd>{dependencyTypeLabel(selectedEdge.dependency_type)}</dd>
                  </div>
                </dl>
                <details className={graphClass('graph-technical-details')}>
                  <summary>Technical details</summary>
...
export function DependencyGraphsPage() {
  ...
  const activeRootId =
    selectedRootId && roots.some((root) => root.id === selectedRootId) ? selectedRootId : (defaultRoot?.id ?? null)
  ...
  return (
    <section className={graphClass('page dependency-graphs-page')}>
      <PageHeader
        ...
        summary={
          <>
            Trace which packages {activeTenant?.name ?? 'the selected tenant'} installs directly and which arrive
      ...
      {rootsQuery.isPending ? (
        <QueryStateNotice
          title="Loading dependency graphs"
          message="Fetching observed install roots for this tenant."
        />
      ) : rootsQuery.isError ? (
        <QueryStateNotice
              ...
              <div className={graphClass('section-header')}>
                <div>
                  ...
                  <p className={graphClass('muted')}>
                    Choose the application or package whose supply chain you want to trace.
                  </p>
                </div>
              </div>
              <div className={graphClass('graph-root-items')} role="group" aria-label="Observed install roots">
                {roots.map((root) => (
                  ...
                  >
                    ...
                    {root.status === 'failed' && root.error ? (
                      <span className={graphClass('graph-root-error')}>{root.error}</span>
                    ) : null}
                  </button>
                ))}
              </div>
                ...
                <>
                  <div className={graphClass('card graph-heading')}>
                    <div>
                      ...
                      <p className={graphClass('muted')}>
                        Explore {graph.nodes?.length ?? 0} packages and {graph.edges?.length ?? 0} relationships from
                        this root.
                      </p>
                      <details className={graphClass('graph-technical-details')}>
                        ...
                        <dl className={graphClass('graph-detail-list')}>
                          <div>
                            <dt>Resolved</dt>
                            <dd>{dateLabel(graph.root.resolved_at)}</dd>
                          </div>
                          <div>
                            <dt>Upstream ID</dt>
                            <dd>
                              <code>{graph.root.upstream_id}</code>
                            </dd>
                          </div>
                          <div>
                            <dt>Graph ID</dt>
                            <dd>
                              <code>{graph.root.id}</code>
                            </dd>
                          </div>
                          <div>
                            <dt>Graph hash</dt>
                            <dd>
                              <code>{graph.root.graph_hash ?? 'Pending'}</code>
                            </dd>
                          </div>
                        </dl>
                      </details>
